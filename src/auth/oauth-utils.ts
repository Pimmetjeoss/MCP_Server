...
/**
 * Sign the client approval and return headers with the signed cookie
 */
export async function signClientApproval(
	clientId: string,
	cookieSecret: string,
	headers: Record<string, string> = {}
): Promise<Record<string, string>> {
	// Get existing approved clients from the provided headers
	...
	// Return headers with Set-Cookie
	return {
		...headers,
		"Set-Cookie": `${COOKIE_NAME}=${newCookieValue}; HttpOnly; Secure; Path=/; SameSite=Lax; Max-Age=${ONE_YEAR_IN_SECONDS}`
	};
...
/**
 * Parse approval form submission and return state with headers
 */
export async function parseApprovalForm(
	request: Request
): Promise<ParsedApprovalResult> {
	if (request.method !== "POST") {
		throw new Error("Invalid request method. Expected POST.");
	...
	try {
		const formData = await request.formData();
		const encodedState = formData.get("state");
		const approved = formData.get("approve") !== null || formData.get("submit") !== null;
